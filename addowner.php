...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["create_owner"])) {
    ...
    if ($result->num_rows > 0) {
        ...
        if ($conn->query($insertLoginQuery) === TRUE) {
            ...
            $insertOwnerQuery = "INSERT INTO hostel_owners1 (hostel_owner,hostel_name, username,password) VALUES ('$hostelname','$hostelname', '$username','$password')";
            
            if ($conn->query($insertOwnerQuery) === TRUE) {
                echo '<script>alert("Hostel owner created successfully!");</script>';
            } else {
